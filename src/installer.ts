...
export async function installValar() {
    ...
    if (arch === 'linux') {
        let toolPath = tc.find('Valar', '0.5.0');

        if (toolPath) {
            core.debug(`Tool found in cache ${toolPath}`);
        } else {
            let url = "http://github.com/valar/cli/releases/v0.5.0/download/valar_linux_amd64";
            let path = await tc.downloadTool(url);
            await exec.exec('chmod', ['+x', path]);
            toolPath = await tc.cacheFile(path, 'valar', 'Valar', '0.5.0', arch);
        }
